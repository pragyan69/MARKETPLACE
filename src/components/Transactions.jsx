...
const Transactions = () => {
    const[transactions]= useGlobalState('transactions')// storing it in empty array
    const [end, setEnd] = useState(3)
    const [count] = useState(3)
    const [collection, setCollection] = useState([])
  
    const getCollection = () => {
      return transactions.slice(0, end) // here the function will return the first 3 transactions
    }
  ...
  return (
   <div  className="bg-[#151c25]">
    <div className="w-4/5 py-10 mx-auto">
        ...
        <h4 className = "text-white text-3xl font-bold uppercase text-gradient"> YOUR LIBRARY</h4>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-4 lg:gap-2 py-2.5">
            {collection.map((trans)=>(
                <div key={tx.id}
              ...
              <p className="text-sm font-medium">{trans.cost}ETH</p>
                </div>
            ))}
            </div>

            {/* now we will create the collection part */}
